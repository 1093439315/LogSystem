...
<script>
    ...
    const {mapActions, mapMutations} = createNamespacedHelpers('errorlog');
    ...
    export default {
        ...
        computed: {
            ...
            pagination() {
                let pagination = this.$store.state.errorlog.pagination;
                if (!pagination) return {DataCount: 0};
                return pagination;
            },
            queryData() {
                let query = this.$store.state.errorlog.queryData;
